...
if (fs.existsSync(quicktype) && process.env.npm_config_production !== "true") {
  ...
  console.log('"Creating types from src/4.0/oa/schema/schema.json"');
  execSync(
    quicktype +
      " -s schema -o src/__generated__/oa-schema.4.0.ts -t OpenAttestationDocument --just-types src/4.0/oa/schema/schema.json --no-date-times"
  );
  console.log('"Creating types from src/4.0/tt/schema/schema.json"');
  execSync(
    quicktype +
      " -s schema -o src/__generated__/tt-schema.4.0.ts -t TradeTrustDocument --just-types src/4.0/tt/schema/schema.json --no-date-times"
  );
} else {
