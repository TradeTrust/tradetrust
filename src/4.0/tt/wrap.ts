...
import { digestCredential } from "../../4.0/tt/digest";
...
export const wrapDocument = async <T extends TradeTrustDocument>(
  ...
  // @context: [Base, OA, ...]
  const contexts = new Set(["https://www.w3.org/2018/credentials/v1", ContextUrl.tt_v4_alpha]);
  ...
  const errors = validate(verifiableCredential, getSchema(SchemaId.tt_v4));
  if (errors.length > 0) {
